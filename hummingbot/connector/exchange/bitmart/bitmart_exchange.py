...
class BitmartExchange(ExchangePyBase):
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        should_add_notional = order_type is OrderType.MARKET and trade_type is TradeType.BUY
        notionalValue: Decimal = amount * Decimal(price)
        api_params = {
            "symbol": await self.exchange_symbol_associated_to_pair(trading_pair),
            "side": trade_type.name.lower(),
            "type": order_type.name.lower(),
            "size": None if order_type is OrderType.MARKET and trade_type is TradeType.BUY else f"{amount:f}",
            "price": f"{price:f}" if order_type is not OrderType.MARKET else None,
            "client_order_id": order_id,
            "notional": f"{notionalValue:f}" if should_add_notional else None,
        }
