...
import hummingbot.connector.exchange.ourbit.ourbit_utils as utils
from hummingbot.connector.exchange.ourbit import ourbit_web_utils as web_utils
...
class OurbitAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def _request_order_book_snapshot(self, trading_pair: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Fallback: convert "BTC-USDT" to "BTCUSDT" format if mapping not available
    ...
    async def _order_book_snapshot(self, trading_pair: str) -> OrderBookMessage:
        ...
        snapshot_msg: OrderBookMessage = OurbitOrderBook.snapshot_message_from_exchange_rest(
            snapshot, snapshot_timestamp, metadata={"trading_pair": trading_pair}
        )
        return snapshot_msg

    async def _parse_trade_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        trading_pair = raw_message.get("s")
        trade_message: OrderBookMessage = OurbitOrderBook.trade_message_from_exchange(
    ...
    async def _parse_order_book_diff_message(self, raw_message: Dict[str, Any], message_queue: asyncio.Queue):
        ...
        order_book_message: OrderBookMessage = OurbitOrderBook.diff_message_from_exchange(
            raw_message, time_now, {"trading_pair": trading_pair}
        )
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        payload = {"method": "ping", "id": int(ping_time * 1e3)}
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            for trading_pair in self._trading_pairs:
                # Convert trading pair to Ourbit symbol format (remove hyphens)
                symbol = trading_pair.replace("-", "").lower()

                # Subscribe to trades
                trade_payload = {"method": "SUBSCRIBE", "params": [f"{symbol}@trade"], "id": 1}
                subscribe_trade_request: WSJSONRequest = WSJSONRequest(payload=trade_payload)

                # Subscribe to depth updates
                depth_payload = {"method": "SUBSCRIBE", "params": [f"{symbol}@depth"], "id": 2}
        ...
        async for ws_response in ws.iter_messages():
            ...
            if data.get("result") is None and "stream" in data:
                ...
                if "@trade" in stream:
                    ...
                    event_data["s"] = symbol
                    self._message_queue[CONSTANTS.TRADE_EVENT_TYPE].put_nowait(event_data)
                elif "@depth" in stream:
                    symbol = stream.split("@")[0].upper()
                    event_data["s"] = symbol
                    self._message_queue[CONSTANTS.DIFF_EVENT_TYPE].put_nowait(event_data)
    ...
    async def _process_ob_snapshot(self, snapshot_queue: asyncio.Queue):
        ...
        while True:
            try:
                ...
                trading_pair = json_msg["s"]
                order_book_message: OrderBookMessage = OurbitOrderBook.snapshot_message_from_exchange_websocket(
